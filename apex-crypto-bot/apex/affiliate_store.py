"""Affiliate tracking — external referrers, per-user isolated."""
import json
import os
import time
import random
...
_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "affiliates")
...
def register(chat_id):
    """Register affiliate. Returns (data, is_new)."""
    chat_id = str(chat_id)
...
def get_by_code(ref_code):
    idx = _load_index()
    chat_id = idx.get(ref_code.upper())
    if not chat_id:
        return None, None
...
def record_sale(ref_code, amount_usd, commission_pct=0.30, product="Apex Bot"):
    """Record sale, return (chat_id, commission) or (None, 0) if code invalid."""
    idx = _load_index()
    chat_id = idx.get(ref_code.upper())
    if not chat_id:
        return None, 0.0
    ...
    data["total_earned"] = round(data.get("total_earned", 0) + commission, 2)
    _save(chat_id, data)
...
def request_payout(chat_id, method, address):
    ...
    if not data:
        return None
    pending = data["total_earned"] - data.get("total_paid", 0)
    if pending < 10:
...
def mark_paid(chat_id, amount):
    ...
    if not data:
        return False
    data["total_paid"] = round(data.get("total_paid", 0) + amount, 2)
