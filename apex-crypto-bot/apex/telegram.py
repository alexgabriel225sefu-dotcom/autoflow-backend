...
def _handle_cb(chat_id, data):
    ...
    if data == "cp:y":
        ...
        if len(parts) == 3:
            _, side, sym = parts
            send_to(chat_id, f"📊 <b>{side} {sym}</b> — choose size:",
...
def _handle_market(chat_id):
    ...
    if not getattr(cfg, 'MARKET_24_7', False) and not forex.is_market_open():
...
_QUICK_SYMS_CRYPTO = ["BTCUSD", "ETHUSD", "SOLUSD", "XRPUSD", "LTCUSD", "BNBUSD"]
_LOT_SIZES_CRYPTO = [0.01, 0.05, 0.1, 0.5, 1.0, 5.0]


def _trade_sym_kb(side):
    """Symbol picker buttons for /buy or /sell."""
    rows = []
    for i in range(0, len(_QUICK_SYMS_CRYPTO), 3):
        row = [{"text": s, "callback_data": f"tr:{side}:{s}"}
               for s in _QUICK_SYMS_CRYPTO[i:i+3]]
        rows.append(row)
    return rows
...
def _trade_lots_kb(side, sym):
    ...
    for i in range(0, len(_LOT_SIZES_CRYPTO), 3):
        row = [{"text": f"{l}", "callback_data": f"tr:{side}:{sym}:{l}"}
               for l in _LOT_SIZES_CRYPTO[i:i+3]]
        rows.append(row)
...
def _handle_buy(chat_id, args):
    ...
    if sym:
        return send_to(chat_id, f"📊 <b>BUY {sym}</b> — choose size:",
                        extra={"reply_markup": {"inline_keyboard": _trade_lots_kb("BUY", sym)}})
    return send_to(chat_id, "📊 <b>BUY</b> — choose a coin:",
...
def _handle_sell(chat_id, args):
    ...
    if sym:
        return send_to(chat_id, f"📊 <b>SELL {sym}</b> — choose size:",
                        extra={"reply_markup": {"inline_keyboard": _trade_lots_kb("SELL", sym)}})
    return send_to(chat_id, "📊 <b>SELL</b> — choose a coin:",
...
def _handle_deploy(chat_id):
    ...
    def _run():
        ...
        if result.returncode != 0:
            ...
            return
        send_to(chat_id, f"✅ <b>Deploy successful!</b> Restarting {cfg.BOT_NAME}...\n\nSend /status when ready.")
        import time
        time.sleep(1)
        subprocess.run("systemctl restart apex-crypto", shell=True, executable="/bin/bash")
