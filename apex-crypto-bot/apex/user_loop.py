...
# nonsense PnL (seen once: an ADAUSD position priced against a ~$75 exit,
# i.e. another symbol's price, producing a six-figure "loss" that never
# happened to the account balance). A same-symbol close during a single
# bot-downtime gap never legitimately moves more than this — treat anything
# past it as an untrustworthy estimate rather than a fact worth alerting on.
_MAX_PLAUSIBLE_GAP_MOVE = 0.5  # 50% — generous even for a bad crypto gap
...
def _nrm(x):
    """Broker-agnostic symbol key (BTC_USD / BTC/USD / btcusd → BTCUSD).
...
def _manage_trailing(broker, cfg, pos, symbol, price, initial_risk=None):
    ...
    pinned the trail a few pips behind price, where ordinary noise closed the
    trade — winners cut at noise distance while losers still ran the full stop.

    None keeps the old derivation, for a position whose opening this process
    never saw (adopted mid-life after a restart with no snapshot, or opened
                ...
                if _deal is not None:
                    ...
                    if exit_price and _snap.get("entryPrice") and units_:
                        if _plausible_exit_price(_snap["entryPrice"], exit_price):
                            ...
                            # exit_price doesn't look like the same symbol as
                            # entryPrice (likely a wrong-instrument candle) —
                            # don't report a fabricated PnL number.
                    ...
                    if maxpos == 1 and watchlist and not open_pos:
                        for ws in watchlist:
                            ...
                            if p_:
                                ...
                                # from the old symbol, a SOL price (~75) compared
                                # against an ADA stop (~0.17) always reads as
                                # "breached", forcing a close whose P&L is computed
                                # from a price that belongs to a different pair
                                # entirely (this produced a six-figure phantom
