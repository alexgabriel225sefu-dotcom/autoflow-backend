...
async function main(){
  ...
  const b = await gql(`mutation($input:VariableUpsertInput!){ variableUpsert(input:$input) }`,
    { input: { projectId: PROJECT_ID, environmentId: ENV_ID, serviceId: SERVICE_ID, name: 'BROKER', value: 'td' } });
  out('BROKER=td: ' + (b?.data?.variableUpsert===true?'OK':JSON.stringify(b).slice(0,200)));
