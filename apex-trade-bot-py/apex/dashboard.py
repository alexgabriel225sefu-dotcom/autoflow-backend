...
<h1>&#9889; Apex Trade Bot <span id="mode" class="badge paper">—</span></h1>
...
<th>PnL</th><th class="hide-sm">PnL %</th><th class="hide-sm">Reason</th></tr></thead>
...
function update(d){
  ...
  $("m-price").textContent="$"+(d.currentPrice||0);
  ...
  $("rows").innerHTML=(d.trades||[]).slice(0,30).map(t=>
    `<tr><td>${esc(t.time)}</td><td>${esc(t.symbol)}</td><td>${esc(t.side)}</td>
     ...
     <td class="hide-sm ${t.win?"up":"down"}">${t.pnlPct>=0?"+":""}${t.pnlPct}%</td>
