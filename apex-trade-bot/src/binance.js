...
function applyFilters(info, quantity, price, symbol = '?') {
  ...
  if (qty < parseFloat(lot.minQty)) throw new Error(`Binance: qty ${qtyStr} below minQty ${lot.minQty} for ${symbol}`);
  const minNotional = parseFloat(notional?.minNotional ?? '0');
  if (price && qty * price < minNotional) {
    throw new Error(`Binance: order value $${(qty * price).toFixed(2)} below minimum $${minNotional} for ${symbol}`);
  }
...
async function placeProtection(symbol, qty, stopLoss, takeProfit) {
  ...
  const params = {
    ...
    belowPrice: round(stopLoss * 0.997),
    belowTimeInForce: 'GTC',
    timestamp: Date.now(),
  };
  const { data } = await client.post(`/orderList/oco?${sign(params)}`, null, { headers: headers() });
  console.log(`[LIVE] 🛡️ OCO placed at exchange: TP ${params.abovePrice} / SL ${params.belowStopPrice} (${symbol})`);
  return data;
}
