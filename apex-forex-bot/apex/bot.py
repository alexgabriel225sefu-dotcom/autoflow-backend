...
def validate():
    ...
    if cfg.BROKER == "mt":
        if not cfg.MT_BRIDGE_SECRET:
            ...
            sys.exit(1)
        print("\U0001f517 MetaTrader bridge mode — waiting for the ApexBridge EA to sync.")
    elif cfg.BROKER == "td":
        if not cfg.TWELVE_DATA_KEY:
            ...
            sys.exit(1)
        print("\U0001f4e1 Twelve Data mode — paper trading with live forex prices.")
...
def open_trade(side, price, balance, atr_value=0, symbol=None, druck_mult=1.0):
    ...
    if units <= 0:
        ...
        return
    if druck_mult != 1.0:
        logger.info(f"\U0001f3af Druckenmiller: position size \xd7{druck_mult:.2f}")
    result = broker.place_order(side, units, symbol, sl=sltp["stopLoss"], tp=sltp["takeProfit"])
...
def best_symbol():
    ...
    if best["sym"] != cfg.SYMBOL:
        logger.info(f"\U0001f4e1 Scanner: best pair → {best['sym']} (score: {best['score']:.2f})")
    return best["sym"]
    ...
    try:
        ...
        if not market_open:
            if not market_closed_alerted:
                logger.info("\U0001f550 Market closed (weekend) — waiting for Sunday open (21:00 UTC)")
        ...
        logger.info(f"[{tick_count}] Analyzing {symbol} ({broker_label()})"
                    f"{' \U0001f512 active position' if active_symbol else ''}...")
        ...
        if stop_check["stop"]:
            logger.warn(f"\U0001f6d1 STRATEGY STOP: {' | '.join(stop_check['reasons'])}")
            now = time.time()
        ...
        if strat_data["livermore"]["trend"] != "NEUTRAL":
            logger.info(f"\U0001f4ca Livermore: {strat_data['livermore']['trend']} "
                        f"({strat_data['livermore'].get('reason')}) | "
                        f"Strength: {strat_data['livermore']['strength'] * 100:.0f}%")
        if strat_data["turtle"]["signal"]:
            logger.info(f"\U0001f422 Turtle: {strat_data['turtle']['breakoutStr']} breakout {strat_data['turtle']['signal']}")
...
def _start_dashboard_server():
    ...
    logger.info(f"\U0001f4ca Dashboard: http://localhost:{port}")
...
def main():
    ...
    mode = "\U0001f4dd PAPER TRADING" if cfg.PAPER_TRADING else (
        "\U0001f517 METATRADER" if cfg.BROKER == "mt" else (
            "\U0001f4e1 TWELVE DATA" if cfg.BROKER == "td" else (
                "\U0001f9ea PRACTICE" if cfg.OANDA_ENV == "practice" else "\U0001f534 LIVE")))
    dash["mode"] = mode.replace("\U0001f4dd", "").replace("\U0001f9ea", "").replace("\U0001f534", "").strip()
    ...
    verify_license()
    logger.info("\U0001f680 First analysis...")
    tick()
