...
def print_signal(signal, ind):
    emoji = {"BUY": "\U0001f4c8", "SELL": "\U0001f4c9", "CLOSE": "\U0001f504"}.get(signal.get("action"), "⏸️")
    print("\n" + "─" * 60)
    ...
    print(f"\U0001f4ad {signal.get('reasoning')}")
    print(f"\U0001f4ca RSI: {ind.get('rsi')} | StochRSI K: {ind.get('stochRsiK')} | MACD Hist: {ind.get('macdHist')}")
    print(f"\U0001f4c8 Trend: {ind.get('emaTrend')} | Structure: {ind.get('marketStructure')} | Divergence: {ind.get('divergence')}")
    print(f"\U0001f4b0 Price: ${ind.get('price')} | ATR: {ind.get('atrPct')}% | Volume: {ind.get('volumeRatio')}x avg")
    if signal.get("keyFactors"):
        print(f"\U0001f511 Key factors: {' • '.join(signal['keyFactors'])}")
    print("─" * 60)
...
def print_stats(balance, open_position=None, current_price=None):
    ...
    mode = "\U0001f4dd PAPER" if cfg.PAPER_TRADING else ("\U0001f9ea PRACTICE" if cfg.OANDA_ENV == "practice" else "\U0001f534 LIVE")
    print("\n\U0001f4ca STATS:")
...
def print_banner(balance):
    mode = "\U0001f4dd PAPER TRADING (no real risk)" if cfg.PAPER_TRADING else (
        "\U0001f9ea OANDA PRACTICE" if cfg.OANDA_ENV == "practice" else "\U0001f534 LIVE TRADING")
    print("\n" + "═" * 60)
    print("  \U0001f4b1 APEX FOREX BOT — AI Powered Trading")
